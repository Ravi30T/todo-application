...
// API-6 Get User Todos Data

app.get('/todos', authenticateToken, async(request, response) => {
    try{
        const {userId} = request
        const getUserId = new ObjectId(userId)

        const todoCollection = client.db(process.env.DB_NAME).collection('tasks');

        const getUserTodos = await todoCollection.find({userId: getUserId}).toArray()

        response.status(201).send(getUserTodos)
    }
    catch(e){
        response.status(500).send({errorMsg: "Invalid User"})
    }
})


// API-7 Update User Details
